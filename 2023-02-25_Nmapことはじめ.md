...
SYNスキャン(-sS)の場合、ターゲットにSYNを送信するだけで判断しますが、こちらはACKまで送信し、TCPコネクションを確立させます。
...
![3ウェイハンドシェイク](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/545707/bca92ec5-db0d-fbf3-7cd6-5aed5d5e87ad.png)
...
多くのファイアウォールは、単に着信パケットをドロップするように設定されています。
そのため、NmapでTCP SYNリクエストを送信しても、何も応答が返ってきません。
これは、ポートがファイアウォールによって保護されていることを示しています。したがって、ポートはフィルタリングされていると考えることができます。
...
[https://www.kagoya.jp/howto/engineer/infosecurity/port-scanning/](https://www.kagoya.jp/howto/engineer/infosecurity/port-scanning/)よ。
...
例えば、**<number>**を**20**としてUDPスキャンした場合、最もよく利用されるTOP20個のUDPポートに対してスキャンを行い、スキャン時間を大幅に短縮できます。
...
TCPスキャンの一種であるNULLスキャン、FINスキャン、Xmasスキャンはそれほぼ一般的でありませんが、ステルススキャンと呼ばれているSYNスキャンより、さらにステルスである傾向があります。
...
スクリプトを新しくインストールする場合は以下の手順を行います。
...
これにより、Windowsのような問題は発生しなくなりますが、スキャンに時間がかかるようになるので注意が必要です。
...
### vulnカテゴリのスクリプト実行(--script=vuln)

http-csrfなど脆弱性を検出するNSEスクリプト群を実行します。
